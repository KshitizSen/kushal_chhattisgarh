        ...
        <table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-gray-600 dark:bg-gray-800 dark:text-gray-300"><tr>{['VT', 'School / VTP', 'Reason', 'HM Status', 'VTP Status', 'Requested', 'Action'].map((h) => <th key={h} className="px-4 py-3 font-medium">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {!loading && requests.map((request) => <tr key={request.id}>
              ...
              <td className="px-4 py-3">{request.status === 'pending' && request[ownStatusKey] === 'pending' ? <div className="flex gap-2"><button disabled={actionId === request.id} onClick={() => openActionModal(request, 'approved')} className="rounded-lg bg-green-600 px-3 py-1.5 text-white disabled:opacity-50">Approve</button><button disabled={actionId === request.id} onClick={() => openActionModal(request, 'rejected')} className="rounded-lg bg-red-600 px-3 py-1.5 text-white disabled:opacity-50">Reject</button></div> : <span className="text-xs text-gray-500">Actioned</span>}</td>
            </tr>)}
