...
const ManageVTP = () => {
  ...
  return <div className="space-y-6">
    ...
    <Card padding="none">{isLoading ? <div className="py-12 text-center text-gray-500 dark:text-gray-400">Loading VTP providers...</div> : <Table columns={columns} data={vtps} renderRow={(vtp, index) => <tr key={vtp.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
      <td className="px-4 py-3 font-medium">{(currentPage - 1) * pageSize + index + 1}</td>
      <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{displayValue(vtp.vtp_name)}</td>
      <td className="px-4 py-3">{displayValue(vtp.vc_name)}</td>
      <td className="px-4 py-3"><span className="flex items-center"><Phone className="mr-2 h-4 w-4 text-gray-400" />{displayValue(vtp.mobile)}</span></td>
      <td className="px-4 py-3"><span className="flex items-center"><Mail className="mr-2 h-4 w-4 text-gray-400" />{displayValue(vtp.email)}</span></td>
      <td className="px-4 py-3"><Badge variant={vtp.status === 'active' ? 'success' : 'danger'} size="sm">{displayValue(vtp.status)}</Badge>
      </td><td className="px-4 py-3"><div className="flex gap-2">
        <Button variant="ghost" size="sm" leftIcon={<Edit className="h-4 w-4" />} onClick={() => openEdit(vtp)}>Update</Button>
        {/* <Button variant="danger" size="sm" leftIcon={<Trash2 className="h-4 w-4" />} onClick={() => setDeleteTarget(vtp)}>Delete</Button> */}
        </div>
      </td>
    </tr>} />}</Card>
    ...
    <Modal isOpen={formModal.open} onClose={closeForm} title={formModal.editing ? 'Update VTP Provider' : 'Add VTP Provider'} size="lg" closeOnOverlayClick={!isSaving} footer={<><Button variant="ghost" onClick={closeForm} disabled={isSaving}>Cancel</Button><Button type="submit" form="vtp-form" loading={isSaving}>{formModal.editing ? 'Update VTP' : 'Add VTP'}</Button></>}>
      <form id="vtp-form" onSubmit={submitForm} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div><label htmlFor="vtp_id" className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">VTP Provider <span className="text-danger-500">*</span></label>
        <select id="vtp_id" value={form.vtp_id} onChange={changeProvider} className={`${selectClass} ${formErrors.vtp_id ? 'border-danger-500' : ''}`} disabled={isSaving}>
          <option value="">Select VTP provider</option>{vtpOptions.map((option) => <option key={option.vtp_id} value={option.vtp_id}>{option.vtp_id} - {option.vtp_name}</option>)}</select>
          {formErrors.vtp_id && <p className="mt-1 text-sm text-danger-500">{formErrors.vtp_id}</p>}
          </div>
        <Input label="VTP Name" value={form.vtp_name} disabled helperText="Filled from VTP master" />
        <Input label="VC/Coordinator Name" required value={form.vc_name} onChange={(event) => changeForm('vc_name', event.target.value)} error={formErrors.vc_name} maxLength={200} disabled={isSaving} />
        <Input label="Mobile Number" required type="tel" inputMode="numeric" value={form.mobile} onChange={(event) => changeForm('mobile', event.target.value.replace(/\D/g, '').slice(0, 10))} error={formErrors.mobile} maxLength={10} disabled={isSaving} />
        <Input label="Email" required type="email" value={form.email} onChange={(event) => changeForm('email', event.target.value)} error={formErrors.email} maxLength={200} disabled={isSaving} />
        <div><label htmlFor="vtp_status" className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Status <span className="text-danger-500">*</span></label>
          <select id="vtp_status" value={form.status} onChange={(event) => changeForm('status', event.target.value)} className={selectClass} disabled={isSaving}>
