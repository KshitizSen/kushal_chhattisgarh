...
const formatDate = (value) => {
  ...
  return !date || Number.isNaN(date.getTime()) ? 'N/A' : date.toLocaleDateString();
};
const validate = (form) => {
  const errors = {};
...
const ManageVTP = () => {
  ...
  const columns = useMemo(() => [
    ...
    { key: 'created_at', label: 'Created' }, { key: 'updated_at', label: 'Updated' }, { key: 'actions', label: 'Action' },
  ], []);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
      <Card variant="filled" padding="md"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500 dark:text-gray-400">Total VC</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{pagination.total}</p></div><UserCog className="h-8 w-8 text-primary-500" /></div></Card>
      <Card variant="filled" padding="md"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500 dark:text-gray-400">Active on Page</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{activeCount}</p></div><CheckCircle className="h-8 w-8 text-success-500" /></div></Card>
      <Card variant="filled" padding="md"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500 dark:text-gray-400">Inactive on Page</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{inactiveCount}</p></div><XCircle className="h-8 w-8 text-danger-500" /></div></Card>
    </div>
    ...
    <Card padding="none">{isLoading ? <div className="py-12 text-center text-gray-500 dark:text-gray-400">Loading VTP providers...</div> : <Table columns={columns} data={vtps} renderRow={(vtp, index) => <tr key={vtp.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
      <td className="px-4 py-3 font-medium">{(currentPage - 1) * pageSize + index + 1}</td><td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{displayValue(vtp.vtp_name)}</td><td className="px-4 py-3">{displayValue(vtp.vc_name)}</td><td className="px-4 py-3"><span className="flex items-center"><Phone className="mr-2 h-4 w-4 text-gray-400" />{displayValue(vtp.mobile)}</span></td><td className="px-4 py-3"><span className="flex items-center"><Mail className="mr-2 h-4 w-4 text-gray-400" />{displayValue(vtp.email)}</span></td><td className="px-4 py-3"><Badge variant={vtp.status === 'active' ? 'success' : 'danger'} size="sm">{displayValue(vtp.status)}</Badge></td><td className="px-4 py-3">{formatDate(vtp.created_at)}</td><td className="px-4 py-3">{formatDate(vtp.updated_at)}</td><td className="px-4 py-3"><div className="flex gap-2"><Button variant="ghost" size="sm" leftIcon={<Edit className="h-4 w-4" />} onClick={() => openEdit(vtp)}>Update</Button><Button variant="danger" size="sm" leftIcon={<Trash2 className="h-4 w-4" />} onClick={() => setDeleteTarget(vtp)}>Delete</Button></div></td>
    </tr>} />}</Card>
